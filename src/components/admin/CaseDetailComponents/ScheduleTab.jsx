    ...
    <div className="bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-10 shadow-sm border border-gray-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
       <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-10 gap-6">
           <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-2xl shadow-inner border border-gray-100">
               <button className="p-2.5 hover:bg-white hover:shadow-sm rounded-xl transition-all text-[#76121F] active:scale-90"><ChevronLeft size={20} strokeWidth={2.5} /></button>
               <div className="px-6 py-2 bg-transparent font-bold text-[#76121F] text-[14px] md:text-[16px] uppercase tracking-widest whitespace-nowrap">March 2026</div>
               <button className="p-2.5 hover:bg-white hover:shadow-sm rounded-xl transition-all text-[#76121F] active:scale-90"><ChevronRight size={20} strokeWidth={2.5} /></button>
           </div>
          ...
          <div className="flex gap-4 min-w-max lg:grid lg:grid-cols-7 lg:min-w-0">
             {weeklySchedule.map((day, idx) => (
               <div key={idx} className="flex flex-col gap-4 w-[160px] md:w-[180px] lg:w-auto">
                  {/* Day Header */}
                  <div className={`rounded-xl p-5 text-center border-2 transition-all ${day.session?.isActiveDay ? 'bg-[#FFBB03]/10 border-[#FFBB03] shadow-lg shadow-[#FFBB03]/5' : 'bg-gray-50 border-transparent opacity-60'}`}>
                     <p className={`text-[12px] font-bold uppercase tracking-[0.2em] leading-none mb-2 ${day.session?.isActiveDay ? 'text-[#76121F]' : 'text-gray-400'}`}>{day.day}</p>
                     <p className={`text-[28px] md:text-3xl font-bold leading-none ${day.session?.isActiveDay ? 'text-[#76121F]' : 'text-[#76121F]/90'}`}>{day.date}</p>
                  </div>
                    ...
                    {day.session ? (
                       <div className="bg-white rounded-[24px] p-5 border border-gray-100 shadow-sm flex flex-col gap-4 group hover:border-[#76121F]/30 hover:shadow-md transition-all duration-300 relative overflow-hidden h-full">
                          ...
                          <div className="flex flex-col gap-1 relative z-10">
                             <h4 className="font-bold text-Third text-[14px] md:text-[15px] leading-tight line-clamp-1">{day.session.client}</h4>
                             <div className="flex items-center gap-1.5 mt-0.5">
                                ...
                                <span className={`text-[10px] uppercase font-bold tracking-widest ${day.session.isActiveDay ? 'text-green-600' : 'text-blue-500'}`}>{day.session.status}</span>
                             </div>
                          </div>
                          
                          <div className="flex flex-col gap-3 relative z-10">
                             <div className="flex items-center gap-2.5 text-gray-400 font-bold text-[11px] md:text-[12px]">
                                <Clock size={14} className="text-[#76121F]/30 shrink-0" /> <span className="truncate">{day.session.time}</span>
                             </div>
                             ...
                             <div className="flex items-center gap-2.5 text-gray-400 font-bold text-[11px] md:text-[12px]">
                                <MapPin size={14} className="text-[#FFBB03]/50 shrink-0" /> <span className="truncate">{day.session.room}</span>
                             </div>
                          </div>
       ...
       <div className="mt-6 text-center lg:hidden">
           <p className="text-[11px] text-gray-300 font-bold uppercase tracking-widest italic">Slide horizontally to explore full weekly schedule</p>
       </div>
    </div>
  );
};
