            ...
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center gap-4">
                  <div className="h-12 w-14 bg-gray-100 rounded-xl animate-pulse shrink-0" style={{ animationDelay: `${i * 0.1}s` }} />
                  <div className="flex-1 space-y-2">
                    <div className="h-4 w-36 bg-gray-200 rounded-full animate-pulse" style={{ animationDelay: `${i * 0.1 + 0.05}s` }} />
                    <div className="h-5 w-20 bg-gray-100 rounded-lg animate-pulse" style={{ animationDelay: `${i * 0.1 + 0.1}s` }} />
                  </div>
                  <div className="h-9 w-24 bg-gray-100 rounded-xl animate-pulse shrink-0" style={{ animationDelay: `${i * 0.1 + 0.15}s` }} />
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm border border-gray-50 flex flex-col gap-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg md:text-xl font-extrabold text-Third">Contribution Center</h2>
              <div className="w-16 h-[3px] bg-Primary rounded-full mt-1" />
            </div>
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider ml-1">Assigned Title</label>
            <input
              type="text"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider ml-1">Asset Selection</label>
            <div className="grid grid-cols-2 gap-3">
              <div
              ...
              >
                ...
                <span className="text-Secondary font-extrabold text-[15px]">Video</span>
                {fileType === "Video" && selectedFile ? (
                  <span className="text-green-600 font-bold text-[10px] text-center leading-tight max-w-full truncate px-1">{selectedFile.name}</span>
                ) : (
                  <span className="text-Secondary/40 font-bold text-[9px] uppercase tracking-widest">MP4, MOV</span>
                )}
              </div>
              ...
              >
                ...
                <span className="text-[#B59654] font-extrabold text-[15px]">Document</span>
                {fileType === "Document" && selectedFile ? (
                  <span className="text-green-600 font-bold text-[10px] text-center leading-tight max-w-full truncate px-1">{selectedFile.name}</span>
                ) : (
                  <span className="text-[#B59654]/40 font-bold text-[9px] uppercase tracking-widest">PDF, DOC</span>
                )}
              </div>
            </div>
            {selectedFile && (
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            >
              {isPending ? "Publishing..." : "Publish Record"} <ArrowUpRight size={17} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm border border-gray-50 flex flex-col max-h-[600px] overflow-hidden">
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div>
              <h2 className="text-lg md:text-xl font-extrabold text-Third">Asset Repository</h2>
          ...
          <div className="flex flex-col gap-3 overflow-y-auto flex-1 custom-scrollbar mt-5">
            {clinicFiles.length > 0 ? (
              clinicFiles.map((file) => {
                ...
                return (
                  <div key={file.id} className="bg-white border border-gray-100 shadow-sm rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 hover:shadow-md hover:border-gray-200 transition-all">
                    <div className="flex items-center gap-4 w-full sm:w-auto min-w-0">
                      <div className="bg-Secondary/[0.03] rounded-xl px-3 py-2 flex flex-col items-center justify-center min-w-[56px] border border-Secondary/5 shrink-0">
                        <span className="text-Secondary/40 font-bold text-[9px] uppercase tracking-tighter leading-none mb-0.5">{month}</span>
                        <span className="text-Secondary font-extrabold text-[18px] leading-none">{day}</span>
                      </div>

                      <div className="flex flex-col gap-1 min-w-0 overflow-hidden">
                        <h3 className="text-[15px] font-extrabold text-Third truncate">{file.title}</h3>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {file.file_size && (
                            <span className="text-[9px] text-gray-300 font-bold">{file.file_size}</span>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-gray-300 font-bold text-[14px]">No assets uploaded yet</p>
                <p className="text-gray-200 text-[11px] font-bold mt-0.5">Publish your first record above</p>
              </div>
            )}
          </div>
