...
const Documents = () => {
  ...
  const { mutate, isPending } = useMutationClient({
    url: "/employee/documents",
    invalidateKeys: [["employeeDocuments"]],
  ...
  return (
    <div className='flex flex-col gap-6 h-full pb-10'>
      ...
      <div className="bg-white rounded-[24px] md:rounded-3xl shadow-sm p-4 md:p-8 flex flex-col min-h-full border border-gray-50">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 text-[13px] md:text-[14px] mt-1">{data?.data?.documents?.length} records currently on file</p>
          </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {data?.data?.documents?.map((doc, index) => (
                <tr key={index} className="hover:bg-gray-50/50 transition-colors">
                  <td className="py-5 px-6 font-bold text-Third text-[14px]">{doc?.document_name}</td>
                  <td className="py-5 px-6 text-gray-500 text-[13px] md:text-[14px] font-medium">{doc?.file_type}</td>
                  <td className="py-5 px-6 text-gray-500 text-[13px] md:text-[14px] font-medium whitespace-nowrap">{doc?.uploaded_date}</td>
                  <td className="py-5 px-6 text-gray-500 text-[13px] md:text-[14px] font-medium whitespace-nowrap">{doc?.expiration_date}</td>
                  ...
                  <td className="py-5 px-6 text-center">
                      ...
                      <button onClick={() => handleDownload(doc?.file_url)} className="inline-flex items-center justify-center gap-2 border border-Secondary/20 text-Secondary hover:bg-Secondary hover:text-white font-bold text-[12px] py-2 px-4 rounded-xl transition-all duration-200 shadow-sm w-full max-w-[140px]">
      ...
      <Dialog open={isUploadModalOpen} onOpenChange={(open) => { if (!open) resetUploadModal(); }}>
        <DialogContent className="max-w-[95vw] sm:max-w-[650px] p-5 sm:p-8 rounded-[24px] overflow-y-auto max-h-[90vh]">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-Third font-bold text-[14px]">Expiration Date *</label>
                <input 
                  type="date" 
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  <div className="text-center">
                    {selectedFile ? (
                      <p className="text-green-600 font-bold text-[15px] sm:text-[16px]">{selectedFile.name}</p>
                    ) : (
                      <>
                        <p className="text-Secondary font-bold text-[15px] sm:text-[16px]">Tap to upload or drag & drop</p>
                        <p className="text-gray-500 text-[12px] mt-1">File, PDF, JPG, PNG (Max 10MB)</p>
                      </>
                    )}
                  </div>
                </div>
