import React from 'react'
import { ImageProvider } from '@/utils/ImageProvider'
import { Link } from 'react-router-dom'
import { motion } from 'motion/react'

const CommonBanner = ({ title, subtitle, link }) => {
  return (
    <div className="bg-[#AD3946] w-full section-padding-x py-16 md:py-24 lg:py-32 xl:py-40 relative overflow-hidden">
      <div className=" flex flex-col md:flex-row items-center justify-between gap-8 mt-14 md:mt-10">
        {/* Left Content */}
        <motion.div 
          initial="hidden"
          animate="visible"
          variants={{
            visible: {
              transition: {
                staggerChildren: 0.15
              }
            }
          }}
          className="flex-1 w-full relative z-10 "
        >
          <motion.p 
            variants={{
              hidden: { opacity: 0, y: 15 },
              visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } }
            }}
            className="text-white font-medium text-sm md:text-base mb-4 tracking-wide"
          >
            <Link to="/" className='hover:text-Primary transition-colors'>Home</Link>  / <span className='text-Primary'>{link}</span>
          </motion.p>
          <motion.h1 
            variants={{
              hidden: { opacity: 0, y: 20 },
              visible: { opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } }
            }}
            className="text-3xl md:text-[44px] lg:text-5xl font-bold text-white mb-6 leading-tight"
          >
            {title}
          </motion.h1>
          <motion.p 
            variants={{
              hidden: { opacity: 0, y: 20 },
              visible: { opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } }
            }}
            className="text-white/90 text-[15px] md:text-lg leading-relaxed max-w-2xl"
          >
            {subtitle}
          </motion.p>
        </motion.div>

        {/* Right Logo (Hidden on smaller screens, visible on md and up) */}
        <motion.div 
          initial={{ opacity: 0, scale: 0.8, rotate: -20 }}
          animate={{ opacity: 1, scale: 1, rotate: 0 }}
          transition={{ duration: 0.8, ease: "easeOut", delay: 0.3 }}
          className="hidden md:flex justify-end relative z-10"
        >
          <img 
            src={ImageProvider.Logo} 
            alt="Overcomers Logo" 
            className="w-40 h-40 md:w-56 md:h-56 lg:w-64 lg:h-64 object-contain animate-spin"
            style={{ animationDuration: '8s' }}
          />
        </motion.div>
      </div>
    </div>
  )
}

export default CommonBanner