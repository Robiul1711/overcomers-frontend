    ...
    <div className="flex items-center justify-between w-full py-4  px-4 md:px-10 bg-[#FAFAFA] border-b border-gray-100 md:border-none sticky top-0 z-[40]">
      <div className="flex items-center gap-3 md:gap-6 min-w-0">
        ...
        {(pathname.includes("/settings") || isCaseDetails) && (
           ...
           >
             <ChevronLeft size={16} className="md:w-[18px]" strokeWidth={2.5}/> <span className="hidden xs:inline">Back</span>
           </button>
        )}

        <div className="flex flex-col min-w-0">
          <h1 className="text-[18px] sm:text-[22px] md:text-[28px] font-bold text-Third leading-tight truncate md:whitespace-normal">{title}</h1>
          <p className="text-gray-500 text-[12px] md:text-[14px] mt-0.5 truncate hidden sm:block">{subtitle}</p>
        </div>
      </div>

      <div className="flex items-center gap-3 md:gap-6 shrink-0">
        <Link to="/dashboard/notifications" className="relative cursor-pointer p-1.5 md:p-0">
        ...
        <Link to="/" className="sm:hidden flex items-center justify-center w-9 h-9 bg-Secondary text-white rounded-lg">
          <Home size={18} />
        </Link>
      </div>
    </div>
