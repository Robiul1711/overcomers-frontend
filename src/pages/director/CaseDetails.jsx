...
const CaseDetails = () => {
  ...
  const { mutate: cancelSchedule } = useMutationClient({
    url: (scheduleId) => `/director/cases/${id}/schedules/${scheduleId}/cancel`,
    method: "put",
    invalidateKeys: [
      ["directorCaseSchedules", id],
      ["directorCaseDetails", id],
    ],
    successMessage: "Schedule cancelled successfully",
  });

  const handleCancelClick = (scheduleId) => {
  ...
  const handleDownloadGraph = async () => {
    const chartElement = document.getElementById("overall-performance-chart-container-director");
    if (!chartElement) {
    ...
    try {
      const canvas = await html2canvas(chartElement, {
        ...
        onclone: (clonedDoc) => {
          ...
          const clonedElement = clonedDoc.getElementById("overall-performance-chart-container-director");
          if (clonedElement) {
                              ...
                              <div className="flex gap-3 pt-2">
                                {task.undoAction ? (
                                  <button
                                    ...
                                    className="flex-1 bg-[#4B5563] text-white py-2 rounded-xl font-bold flex items-center justify-center gap-1.5 hover:bg-[#374151] transition-all animate-in fade-in zoom-in duration-300 text-xs cursor-pointer shadow-sm"
                                  >
                                    <RotateCcw size={16} />
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="col-span-1 xl:col-span-2 bg-white rounded-3xl p-4 md:p-6 shadow-sm border border-gray-100 flex flex-col">
                  ...
                  {isPerformanceLoading ? (
                    ...
                    <div id="overall-performance-chart-container-director" className="w-full h-[280px] bg-white">
