...
const Cases = () => {
  ...
  const filteredData = selectedStatus === "All Statuses"
    ? casesData
    : casesData.filter((item) => item.status.toLowerCase() === selectedStatus.toLowerCase());
    ...
    <div className="flex flex-col gap-6 h-full font-poppins">
      ...
      <div className="bg-white rounded-[24px] md:rounded-3xl shadow-sm p-4 md:p-8 flex flex-col min-h-full border border-gray-50">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
          <div>
            <h3 className="text-[20px] md:text-[24px] font-bold text-Third">Assigned Cases</h3>
            ...
            <table className="w-full text-left border-collapse min-w-[850px]">
              <thead>
                <tr className="bg-gray-50/80">
                  <th className="py-4 px-6 font-bold text-Third text-[13px] uppercase tracking-wider rounded-tl-xl">
                    Client Name
                  </th>
                  ...
                  <th className="py-4 px-6 font-bold text-Third text-[13px] uppercase tracking-wider rounded-tr-xl text-center">
                    Action
                  </th>
                </tr>
              </thead>
                ...
                {filteredData?.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-5 px-6 text-center">
                      <span
                        className={`px-4 py-1.5 rounded-full text-[11px] font-bold border border-transparent shadow-[0_2px_10px_rgba(0,0,0,0.02)] ${getStatusStyles(
                          item?.status
                        )}`}
                      >
                        {item?.status}
                    ...
                    <td className="py-5 px-6 text-center">
                      <Link to={`/supervisor-dashboard/cases/${item?.id}`} className="inline-block min-w-[120px]">
