...
import { Upload, Download, UploadCloud, FileText, CheckCircle2, Clock } from "lucide-react";
...
const SupervisorDocuments = () => {
  ...
  // Fetch supervisor documents
  const { data, isLoading } = useClient({
    queryKey: ["supervisorDocuments"],
    url: "/supervisor/documents",
  });

  const documentsList = data?.data?.documents || [];
  const totalQuantity = data?.data?.total_quantity || 0;
  const uploadedQuantity = data?.data?.uploaded_quantity || 0;
  const pendingQuantity = data?.data?.pending_quantity || 0;

  // Download handler
  const handleDownload = async (docId, fileName) => {
    try {
      const response = await axiosSecure.get(
        `/supervisor/documents/${docId}/download`,
        { responseType: "blob" }
      );
  ...
  const { mutate, isPending } = useMutationClient({
    url: "/supervisor/documents",
    invalidateKeys: [["supervisorDocuments"]],
    ...
    return (
      <div className="flex flex-col gap-6 h-full pb-10">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm animate-pulse flex items-center gap-4">
    ...
    <div className="flex flex-col gap-6 h-full pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((item, idx) => (
          <div
            key={idx}
            className="bg-white rounded-xl p-5 shadow-sm border border-gray-50 flex items-center gap-4 hover:shadow-md transition-shadow"
          >
            <div className={`w-12 h-12 rounded-xl ${item.bgColor} flex items-center justify-center shrink-0`}>
              {item.icon}
            </div>
            <div className="flex flex-col gap-0.5">
              <h3 className="text-2xl font-extrabold text-Third leading-none">
                {item.value}
              </h3>
