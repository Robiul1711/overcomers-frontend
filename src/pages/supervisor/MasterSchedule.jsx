...
const SupervisorMasterSchedule = () => {
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [selectedStatus, setSelectedStatus] = useState("All Statuses");
  const [searchQuery, setSearchQuery] = useState("");

  const { data, isLoading, isError } = useClient({
    queryKey: ["supervisorMasterSchedule"],
    url: "/supervisor/schedules/master-schedule",
  });
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
            <p className="text-rose-500 font-bold text-sm">
              Failed to load master schedule records.
            </p>
                  ...
                  >
                    ...
                    <td className="py-5 px-6 text-gray-500 text-[13px] md:text-[14px] font-medium">
                      <div className="flex flex-col gap-1 max-w-[200px]">
                        <span className="text-Third font-bold text-[13px]">
                          {event.session_type}
                        </span>
                        {event.location && (
                    ...
                    <td className="py-5 px-6 text-center">
                      <Link
                        to={`/supervisor-dashboard/cases/${event.clinical_case_id}`}
...
export default SupervisorMasterSchedule;
