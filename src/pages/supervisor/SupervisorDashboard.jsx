...
import { Briefcase, Clock, Users, Calendar, Activity, TrendingUp } from "lucide-react";
...
const SupervisorDashboard = () => {
  ...
  const message = resData?.message || "Welcome to Supervisor Dashboard";
  ...
  const stats = [
    {
      ...
      description: "Active clinical cases under supervision",
    },
    {
      ...
      description: "Sessions awaiting your validation",
    },
    {
      ...
      description: "Therapists and RBTs on caseload",
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8 font-poppins text-Third w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl shadow-sm border border-gray-50">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">Supervisor Dashboard</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-50 shadow-sm flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              <Briefcase size={20} className="text-Secondary group-hover:scale-110 transition-transform" />
              <span className="font-bold text-sm">Manage Cases</span>
              <span className="text-xs text-gray-400 font-medium">Oversight & active programs</span>
            </Link>
            <Link
              to="/supervisor-dashboard/sessions"
              className="p-4 bg-gray-50 hover:bg-Primary/10 border border-gray-100 hover:border-Primary rounded-2xl transition-all flex flex-col gap-2 group"
            >
              <Clock size={20} className="text-Secondary group-hover:scale-110 transition-transform" />
              <span className="font-bold text-sm">Pending Sessions</span>
              <span className="text-xs text-gray-400 font-medium">Validate therapist logs</span>
            </Link>
            <Link
              to="/supervisor-dashboard/profile"
              className="p-4 bg-gray-50 hover:bg-Primary/10 border border-gray-100 hover:border-Primary rounded-2xl transition-all flex flex-col gap-2 group"
            >
              <Users size={20} className="text-Secondary group-hover:scale-110 transition-transform" />
              <span className="font-bold text-sm">My Profile</span>
              <span className="text-xs text-gray-400 font-medium">Manage details & credentials</span>
            </Link>
          </div>
        </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-50 shadow-sm flex flex-col justify-between gap-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold">Clinical Supervision Insights</h2>
            <TrendingUp size={18} className="text-emerald-600" />
          </div>
          <p className="text-sm text-gray-500 font-medium leading-relaxed">
            Welcome to your Supervisor Dashboard. As a BCBA / Supervisor, you can oversee ongoing client programs, track therapist session logs, approve data collections, and manage clinical progress across your assigned caseload.
          </p>
          <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 bg-emerald-50 border border-emerald-100 px-3 py-2 rounded-xl w-fit">
            <TrendingUp size={14} /> Caseload compliance is currently up-to-date.
          </div>
        </div>
      </div>
    </div>
